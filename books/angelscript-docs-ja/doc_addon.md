...
`CScriptBuilder` はスクリプトのロードとビルドを簡素化するためのヘルパークラスで、条件付きコンパイル、インクルードディレクティブ、pragma ディレクティブ、メタデータ宣言などをサポートした基本的なプリプロセッサを備えています。

デフォルトでは、スクリプトビルダーはインクルードされたファイルがインクルードされたファイルの相対ディレクトリからロードすることでインクルードディレクティブを解決します。別の方法でインクルードを処理したい場合は、インクルードコールバックを実装してください。

アプリケーションが pragma ディレクティブをサポートする場合は、pragma コールバックを登録してください。コールバックがないと、pragma ディレクティブに遭遇した際にスクリプトビルダーはエラーを報告します。

スクリプトビルダーはまた `#!` で始まる行をコメントとして削除します。これは Linux や UNIX ベースの OS でよく使用される shebang インタープリタディレクティブをサポートするためです。
...
ビルダーは `#if`/`#endif` プリプロセッサディレクティブによる条件付きコンパイルをサポートしています。アプリケーションは `DefineWord()` の呼び出しで単語を定義でき、スクリプトはこの定義をコードでチェックして部分を含めたり除外したりすることができます。
...
`CContextMgr` は並行して実行される複数のスクリプトの管理を補助するために設計されたクラスです。並行スクリプトスレッドとコルーチンの両方をサポートしています。

アプリケーションが複数のコンテキストを必要としない場合（つまり次のスクリプトが実行される前にすべてのスクリプトが完了する場合）、このクラスは必要ありません。

コンテキストマネージャーはデバッグやプールのためにエンジンに登録されたコンテキストコールバックを活用するために `asIScriptEngine::RequestContext` を使用します。

複数のコンテキストマネージャーを使用することができます。例えば、ゲーム内オブジェクトを制御するスクリプトのグループとGUI要素を制御するスクリプトのグループがある場合、これらの各グループは異なるコンテキストマネージャーによって管理されます。

コンテキストマネージャークラスはマルチスレッド向けには設計されていないため、アプリケーションが複数のスレッドからスクリプトを実行する場合は注意が必要です。

参照: サンプルの [concurrent](./doc_samples#concurrent-scripts) および [corout](./doc_samples#co-routines)
...
スクリプトインターフェースについては [コルーチン](./doc_script_stdlib#コルーチン-co-routines) を参照してください。
...
`CDebugger` はスクリプトのデバッグ（ブレークポイントの設定、コードのステップ実行、変数値の検査など）のための一般的なデバッグ機能を実装しています。

デバッガーを使用するには、コンテキストにラインコールバックを設定する必要があります。これにより、ブレークポイントに達したときにデバッガーが制御を引き継ぎ、スクリプトをデバッグできます。

デフォルトでは、デバッガーは標準入力と標準出力ストリームを使用してユーザーと対話しますが、`CDebugger` クラスから派生して `TakeCommands` と `Output` メソッドを実装することで簡単にオーバーロードできます。これにより、グラフィカルインターフェースや、アプリケーションのリモートデバッグを実装することが可能です。
...
`CSerializer` はモジュール内のグローバル変数の値のシリアライズのサポートを実装します。例えば、すべてを再初期化せずにわずかに変更されたバージョンのスクリプトをリロードするためです。参照とハンドルを含むプリミティブとスクリプトクラスは自動的に解決されます。

アプリケーション登録型については、これらをシリアライズする方法を示すコールバックオブジェクトを実装する必要があります。POD型は例外で、シリアライザーは単にビットごとのコピーを保持します。ファクトリのない登録参照型はシリアライズされません。

**現在の制限事項:**
- メモリへのシリアライズのみ可能（ファイルへの保存は不可）
- 復元時に変数の型が変更された場合、シリアライザーは値を復元できません
- シリアライザーはすべてのオブジェクトをバックアップしようとしますが、実際のオブジェクトではなくそれへの参照のみをバックアップしたい場合がある（例えばスクリプトから参照されるアプリケーション内部オブジェクト）。現在これをシリアライザーに指示する方法はありません
- モジュールが別のモジュールのオブジェクトへの参照を保持している場合、値の復元に失敗することがあります
...
このヘッダーファイルは、アプリケーション開発者がプリプロセッサマクロへの簡単な呼び出しで汎用呼び出し規約を使用したラッパー関数を自動的に生成できるようにするマクロとテンプレート関数を宣言します。これはネイティブ呼び出し規約がまだサポートされていないプラットフォームに有用です。
...
スクリプトインターフェースについては [例外処理](./doc_script_stdlib#例外処理-exception-handling) を参照してください。
...
このアドオンは `std::string` 型をそのまま AngelScript に登録します。これにより、パラメータまたは戻り値の型として `std::string` を使用する C++ 関数との完全な互換性が得られます。

`std::string` 型は値型であるため、スクリプトコードで文字列値が渡される際にコピーの数が増加する可能性がある点が潜在的なデメリットです。ただしこれは多くの文字列操作を行うスクリプトでのみ問題になります。
...
`array` 型はスクリプトが任意の型の配列を宣言できるようにする[テンプレートオブジェクト](./doc_adv_template)です。汎用クラスであるため、ランタイムで特性を決定する必要があり、最も高パフォーマンスではありません。そのため、最も頻繁に使用される配列型についてはテンプレートの特殊化を登録することをお勧めします。
...
`grid` 型はスクリプトが任意の型の2次元グリッドを宣言できるようにする[テンプレートオブジェクト](./doc_adv_template)です。多くの点でarrayに似ていますが、エリアでの使用に特化しています。
...
`any` 型は任意の値を保持できる汎用コンテナです。参照型です。
...
`ref` 型は任意のハンドルを保持できる汎用コンテナです。値型ですが、オブジェクトハンドルと非常に似た動作をします。
...
`weakref` 型はオブジェクトへの弱参照（参照されるオブジェクトを生かし続けない参照）を保持するためのテンプレート型です。
...
辞書オブジェクトは文字列の値を他の型の値またはオブジェクトにマッピングします。
...
このオブジェクトはファイルシステム上のディレクトリの検査のサポートを提供します。
...
このアドオンは標準 C ランタイムライブラリの数学関数をスクリプトエンジンに登録します。`RegisterScriptMath(asIScriptEngine*)` を使用して登録を実行します。
