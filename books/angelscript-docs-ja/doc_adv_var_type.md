...
関数をこの特殊なパラメータ型で登録すると、その関数には参照と追加の引数としてその変数の [型ID（Type ID）](./doc_understanding_as#typeid-の構造) が渡されます。この参照は、呼び出し元が渡した「実体」を指します。例えば、渡された式がオブジェクトハンドルである場合、参照はオブジェクトそのものではなく、ハンドル自体を指すことになります。
...
例については [ハンドルアドオン](./doc_addon#ref-オブジェクト) および [辞書アドオン](./doc_addon#dictionary-オブジェクト) を参照してください。
