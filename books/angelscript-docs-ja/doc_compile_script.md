...
AngelScript はファイル自体をロードしないため、スクリプト内から他のファイルをインクルードするための組み込みサポートもありません。しかし、アドオンディレクトリにある [CScriptBuilder](./doc_addon#スクリプトビルダー-script-builder) クラスを利用すれば、これらのサポートを受けることができます。これはファイルのロード、プリプロセスの実行、そしてモジュールのビルドを行うための便利なヘルパークラスです。スクリプトビルダーの使用例は、[Hello World](./doc_hello_world) で確認できます。
