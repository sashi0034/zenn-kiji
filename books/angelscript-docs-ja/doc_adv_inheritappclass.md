...
アプリケーションクラスがネイティブの機械語にコンパイルされるのとは異なり、スクリプトクラスはネイティブな機械語にコンパイルされないため、スクリプトクラスはアプリケーションが登録したクラスから直接継承することはできません。

しかし、抽象化レイヤーの背後にある違いを隠蔽するプロキシクラス (proxy class) を使用することで、継承をエミュレート（模倣）することは可能です。プロキシクラスは2つの部分から構成されます。1つはアプリケーションが見る C++ 側であり、もう 1つはスクリプトが見て継承できるスクリプト側です。
...
もちろん、アプリケーションからスクリプト化されたクラスのインスタンスを作成し、`FooScripted` C++ プロキシを通じてそれにアクセスすることも可能であり、こうすることでアプリケーションの他の部分に対して、実装が実際にはスクリプト内で行われているという事実を透過的（隠蔽されている状態）にすることができます。
