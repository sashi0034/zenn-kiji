...
複数のスクリプトを並行して実行するためには、各スクリプトが独自の `asIScriptContext` を持っていなければならず、[通常の関数呼び出し](./doc_call_script_func) と同じようにコンテキストがセットアップされます。その後、アプリケーションは各コンテキストに対してタイムアウトを設定する必要があります。タイムアウトに達した場合、コンテキストはサスペンド（一時停止）され、次のコンテキストがしばらくの間実行できるようにすべきです。
...
上記の例の `SetTimeoutForContext` のようなタイムアウト関数は、2つの異なる方法で実装することができます。1つは [ラインコールバック](./doc_debug#ラインブレークポイントの設定-setting-line-breaks) を使用する方法で、コンテキストはスクリプト内の各ステートメントに対してコールバックを呼び出します。そこでコールバックがタイムアウトリミットに達したかどうかを確認し、コンテキストをサスペンドさせることができます。
