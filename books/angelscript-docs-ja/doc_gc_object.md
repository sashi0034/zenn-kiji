---
title: "🛠️ ガベージコレクション対応オブジェクト (Garbage collected objects)"
---

メモリ管理手法としての参照カウントには、循環参照が発生した場合に不要になったオブジェクト（デッドオブジェクト）を検出できないという弱点があります。AngelScript では、循環参照を検出して適切にメモリを解放する「ガベージコレクション (GC)」の仕組みをサポートするため、特別な振る舞い（behaviours）を持つ型を登録できるようになっています。これらの振る舞いを実装するとクラスの設計はやや複雑になりますが、登録が必要なのは汎用的なコンテナクラスなど、ごく一部の型に限定されるはずです。
...
ガベージコレクション対応型と非対応型の主な違いは、`addref` および `release` の振る舞い、クラスコンストラクタ、そして GC 用の追加サポート用振る舞いの有無にあります。

ガベージコレクション対応オブジェクトの具体的な実装例については、[辞書 (dictionary)](./doc_addon#dictionary-オブジェクト) アドオンを参照してください。

ガベージコレクター (GC) は、各オブジェクトから辿ることのできる参照の経路を詳しく調べることで、オブジェクトを破棄すべきタイミングを判断します。GC があるオブジェクトへのすべての参照を把握できた場合、そのオブジェクトは循環参照の一部であるとみなされます。その循環参照に関与するすべてのオブジェクトに外部からの参照が存在しない場合、それらはすべて破棄の対象となります。

デッドオブジェクトの判定には、以下の振る舞いのうち最初の4つが使用されます。また、最終的なオブジェクトの破棄は、そのオブジェクトが保持している参照を強制的に解放 (Release) させることで実行されます。
...
{
    // この呼び出しが行われたとき、オブジェクトはすでに破棄対象となっていますが、
    // GC がまだこのオブジェクトへの参照を保持しているため、直ちに delete することはできません。
    // ここでは、このオブジェクトが保持している他のオブジェクトへのすべての参照を解放します。
    if( myref )
    {
        myref->Release();
        myref = 0;
...
参照: GCの振る舞いを持つ値型の例としての [handle アドオン](./doc_addon#ref-オブジェクト)、およびGCの振る舞いを持つ値型を含むことができる参照型の例としての [dictionary アドオン](./doc_addon#dictionary-オブジェクト)。
