...
ライブラリのサイズは、コンパイラの種類、コンパイラフラグ、AngelScript のどの機能が含まれているかなど、様々な要因によって異なります。しかしながら、ライブラリがディスクとメモリ上でどれだけのスペースを占めるかの目安を示すために、[asrun サンプル (asrun sample)](./doc_samples#command-line-runnerコマンドラインランナー) をいくつかの異なる方法でコンパイルし、サイズを記録しました。
