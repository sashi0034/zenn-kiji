...
[多重継承](./doc_script_class#継承とポリモーフィズム-inheritance-and-polymorphism) が利用できないため（AngelScript は単一継承のみをサポートしています）、複数のクラスに同一のコードを実装しなければならないことがあります。これが必要な場合は、同じコードを複数の場所に重複して記述することを避けるために、ミックスインクラスの使用が推奨されます。
