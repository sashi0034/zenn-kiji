...
[関数のインポート](./doc_script_global#関数のインポート-imports) は、スクリプトモジュール間でコードを [共有](./doc_script_shared) するための手法の一つです。ただし、`shared` キーワードによる共有とは異なり、`import` を使用する場合は、スクリプトのコンパイル後にインポートされた関数をバインドするための専用の処理をアプリケーション側に実装する必要があります。
