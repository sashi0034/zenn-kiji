...
新しいオブジェクト型を登録する場合、大きく分けて 2 つのアプローチがあります。一つは、ヒープ領域などの動적メモリに確保される**参照型 (Reference Type)**、もう一つは、スタック上や他のオブジェクトのメンバとして直接配置される**値型 (Value Type)**です。参照型は（アプリケーションによる制限がない限り）オブジェクトハンドルをサポートしますが、値渡しでやり取りすることはできません。一方、値型はハンドルをサポートしませんが、値渡し（Pass-by-value）や参照渡しが可能です。
