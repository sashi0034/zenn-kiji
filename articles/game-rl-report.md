...
皆さんはゲームで AI を作るとなると、普通はルールベースのプログラムで制御すると思います。「プレイヤーが距離 100 以内に来たら攻撃」のように開発者が予め行動を記述した AI ですね。
...
今回、個人的な好みでレンダリングには Siv3D というエンジンを使用しました。Siv3D は C++ で手軽に 2D/3D 描画や入力処理を扱えるライブラリで、今回のような小規模なゲームプロトタイプを作るにはかなり便利です。
...
今回の教訓として、強化学習 AI をゲームに導入するのであれば、ゲーム側もそれ相応の設計を意識すべきであるという学びを得ました。特に、以下の点が重要だと思います。
