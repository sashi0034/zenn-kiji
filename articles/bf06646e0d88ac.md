...
[C++ Advent Calendar 2025](https://qiita.com/advent-calendar/2025/cxx) 18 日目の記事です。
以前も何度か AngelScript に関する記事[^previous-article]を書いたのですが、そういえば実際に C++ プロジェクトへ導入する方法を説明しきれてなかったなと思い至ったので、自分の備忘録も兼ねて今回まとめてみようと思います。所々過去の記事内容と重複箇所があって恐縮ですが、改めて整理してみます。

# 2. AngelScript について

![angelscript](/images/angelscript-project-2025/angelscript.png)
...
C++ と似た文法と書いてありますが、delegate や setter / getter のように C#-like な仕様も混ざっているのが特徴です。C++ と同様に RAII 的なことも出来ます。

有名な採用事例として、**2021 年に GOTY を受賞した『It Takes Two』では AngelScript が使われていました**[^it-takes-two]。開発元であるスウェーデンのゲーム会社 Hazelight は Unreal Engine に AngelScript を組み込んだフォーク[^unreal-angelscript]を開発しています。
...
![image-20251210171157611](/images/angelscript-project-2025/image-20251210145428836.png)
...
C++ と AngelScript で数値型を一対一対応することが出来るので、AngelScript から C++ 関数を呼び出す際はC++でラッパー関数を経由せずとも直接呼ぶことが可能です (「ネイティブ呼び出し」と言うようです)。C++ と同一の型体系を持つことで、スクリプトとネイティブコードの境界におけるオーバーヘッドを最小化できます。

数値型を厳密に区別することは、非エンジニアにとって学習コストを高めてしまう側面はあるものの、性能要件の厳しい大規模ゲーム開発においては大きな効果を発揮しそうです。

こうした特性に加えて、**AngelScript は実行時にネイティブコードを生成する (JIT コンパイラ) ためのインターフェースが備わっており、スクリプト実行であっても JIT を最大限活用できれば大幅にパフォーマンスを向上させることが出来ます**[^as-jit][^as-jit-old-repos][^as-jit-new-repos]。 Lua でコーディングする場合はボトルネック部分を後から C++ で書き直すという手間が生じることもありますが、そもそもスクリプト自体の性能が十分であるならば C++ への書き換えを意識しなくてもよくなりそうです。
...
導入について、CMake プロジェクトで設定する場合、AI エージェント丸投げで問題なさそうでした。

しかしながら、Windows / Visual Studio プロジェクトの場合、人力で GUI から設定をぽちぽちするのはややこしいので以下に手順を掲載します。⬇️


## Visual Studio プロジェクトで AngelScript を導入する
...
![image-20251210172559784](/images/angelscript-project-2025/image-20251210171143947.png)

4. ソリューションエクスプローラーから '自分のプロジェクト名' を右クリック
5. 「追加」➡️「参照」➡️angelscript にチェックを入れて OK

![image-20251210172804915](/images/angelscript-project-2025/image-20251210172804915.png)

![image-20251210172846492](/images/angelscript-project-2025/image-20251210172846492.png)
...
![image-20251210173706977](/images/angelscript-project-2025/image-20251210173134444.png)
...
![image-20251210174627266](/images/angelscript-project-2025/image-20251210174627266.png)

---

本記事のために作った Visual Studio プロジェクトは以下のリポジトリに置きました。

https://github.com/sashi0034/AngelScript-MinimalExample
...
公式ページ[^doc-register-func]には以下のような例が示されています。
...
int r = engine->RegisterObjectType("object", sizeof(Object), asOBJ_VALUE | asGetTypeTraits<Object>()); assert(r >= 0);
...
また、C++ 側から AngelScript の関数呼び出しは `asbind20::script_invoke()` で簡潔に書けます。

```cpp
// .cpp

// my_script.as で定義された as_main() を取得
asIScriptFunction* func =
    module->GetFunctionByDecl("float as_main(FlagAndVector3 value, GameObject& obj)");

// 引数を渡して実行する
const auto result = asbind20::script_invoke<float>(ctx, func, arg_value, &arg_obj);
if (result.has_value())
{
    // 成功: 関数の戻り値は result.value() で取得
...
実際にプログラムを動作させてみましょう。サンプルコード全体を以下に掲載します。フォルダ構成は先程と同様です。
...
![image-20251214150210609](/images/angelscript-project-2025/image-20251214150210609.png)
...
2. スクリプトが入ったディレクトリの直下に `as.predefined` というファイルを作成します。
   ...
   ![image-20251214151210550](/images/angelscript-project-2025/image-20251214151210550.png)
...
4. `my_script.as` を開けて、実際にコーディングしてみます。
   ...
   ![image-20251214152435518](/images/angelscript-project-2025/image-20251214152435518.png)
...
[^previous-article]: Lua の代替スクリプト言語まとめ (https://zenn.dev/sashi0034/articles/94ef379f8dbd71)
[^official-angelcode]: AngelCode.com (https://www.angelcode.com/)
[^it-takes-two]: スウェーデン発のヒット作「It Takes Two」レベルデザイナーが語る。デザイナーとプログラマー，役職の垣根を超えたクリエイティブな環境 (https://www.4gamer.net/games/514/G051498/20220425103/)
[^unreal-angelscript]: Unreal Engine Angelscript (https://angelscript.hazelight.se/)
...
[^as-jit]: AngelScript: How to build a JIT compiler (https://www.angelcode.com/angelscript/sdk/docs/manual/doc_adv_jit.html)
[^as-jit-old-repos]: 古い JIT コンパイラとして https://github.com/BlindMindStudios/AngelScript-JIT-Compiler があります。
[^as-jit-new-repos]: 調べてみたら新しい JIT コンパイラを開発しているプロジェクトがありました: https://github.com/asumagic/angelsea
